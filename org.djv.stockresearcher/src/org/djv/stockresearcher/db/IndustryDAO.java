...
public class IndustryDAO extends H2DAO{
	...
	private static final String CREATE_SQL = "CREATE TABLE IF NOT EXISTS INDUSTRY (ID INTEGER, SECTORNAME CHAR(40), INDUSTRYNAME CHAR(40), DATADATE DATE)";
	private static final String SELECT_SQL = "SELECT SECTORNAME, INDUSTRYNAME, DATADATE FROM INDUSTRY WHERE ID = ?";
	
	private static final String SELECT_ID_FOR_SECTOR_SQL = "SELECT ID FROM INDUSTRY WHERE SECTORNAME = ?";
	
	private static final String SELECT_ALL_SECTOR_SQL = "SELECT DISTINCT(SECTORNAME) AS SNAME FROM INDUSTRY ORDER BY SNAME";
	private static final String SELECT_INDNAME_FOR_SECTOR_SQL = "SELECT INDUSTRYNAME FROM INDUSTRY WHERE SECTORNAME = ?";
	private static final String SELECT_ID_FOR_SECTOR_IND_SQL = "SELECT ID FROM INDUSTRY WHERE SECTORNAME = ? AND INDUSTRYNAME = ?";
	
	private static final String UPDATE_SQL = "UPDATE INDUSTRY SET SECTORNAME = ?, INDUSTRYNAME = ?, DATADATE = ? WHERE ID = ?";
